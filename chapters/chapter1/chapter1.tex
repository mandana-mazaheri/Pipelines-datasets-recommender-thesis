...
% what the problem is
% Currently, the majority of studies, research and their corresponding resources and implementations are publicly accessible. These resources also includes the scientific datasets and analysis tools in the area of neuroscience. Therefore, there has been some openly accessible platforms developed, such as OpenNeuro, through which people can find, download, share and analyze the neuroscience resources such as datasets, tools or analysis pipelines. Although these platforms help users in many aspects, the users still are unassisted in finding the best pairs of tool and data for their analysis. In this thesis, we investigate the feasibility of implementing a recommender system for neuroimaging datasets and analysis pipelines given the other, based on the previous executed pairs of pipeline-dataset.
% % In the rest of introduction, we will have an overview on the key concepts of this thesis and will explain the goal of the thesis in more details. 
...
% \TG{Make sure that all references are complete (authors, title, date, journal or conference name, pages, volume/number for journal papers. }
